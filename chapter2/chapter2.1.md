...
- B*Tree 인덱스에서 B는 Balanced의 약자이다.
...
### 2.1.4 인덱스 수평적 탐색
> 수직적 탐색은 "어디서부터(Start Point)"를 찾는 것이고, 수평적 탐색은 시작점부터 "어디까지(End Point)" 만족하는지 훑으며 ROWID를 챙기는 과정입니다.

- 인덱스에서 본격적으로 데이터를 찾는 과정이다. 
- Leaf 블록은 서로의 앞뒤 블록에 대한 주소값을 갖는다. 
- 인덱스를 수평적으로 탐색하는 이유는?
    1. 조건절을 만족하는 데이터를 모두 찾기 위해서
		- B*Tree의 루트와 브랜치는 길잡이일 뿐입니다. 
		- 실제 데이터(혹은 주소)는 리프 노드에 정렬된 상태로 모여 있습니다.
		- 인덱스는 중복 값을 가질 수 있습니다. (예: 성이 '이'씨인 사람 100명)
		- 수직적 탐색으로 '이'씨의 시작점을 찾았다면?
			1. 그다음 '이'씨가 아닌 사람('최'씨 등)이 나올 때까지 
			2. 리프 노드를 옆으로 쭉 읽어야 
			3. 모든 데이터를 놓치지 않고 가져올 수 있습니다.
    2. 테이블 액서스하기 위한 ROWID를 얻기 위해서
		- 인덱스만 봐서 끝나는 경우(`SELECT COUNT(*)` 등)도 있지만
		- 보통은 이름, 주소 같은 나머지 정보가 필요합니다.
		- 리프 노드에는 `[키값 | ROWID]`가 쌍으로 들어있습니다.	
		- 수평적으로 이동하며 조건에 맞는 모든 `ROWID(실제 데이터 주소)` 를 수집해야, 
		- 나중에 테이블로 찾아가서 진짜 데이터를 가져올 수 있습니다.
	3. 왜 위로 다시 안 올라가고 옆으로 가나요?
		- B*Tree의 리프 노드들은 Double Linked List(양방향 연결 리스트) 구조로 서로의 주소를 알고 있습니다.
 		- 만약 옆으로 가는 길이 없다면? 
			- 10번 읽고 다시 브랜치로 올라가서 11번 위치 물어보고 다시 내려와야 합니다. (매우 비효율적)
		- 옆으로 가는 길이 있어야 하는 이유
			- 한 번만 내려가면, 그다음부터는 기차 칸을 건너가듯 옆 블록으로 바로바로 이동할 수 있습니다. 
			- 이것이 인덱스가 대량의 범위 탐색에 강력한 이유입니다.
	4. Index Leaf Scan의 종료 조건
		- 인덱스를 옆으로 쭉 읽다가 **"이제 그만 읽어도 되겠다"** 라고 판단하고 멈추는 기준은 무엇일까?
		- 인덱스를 수평적으로 읽다가 멈추는 기준은 바로 `"나보다 큰 놈(또는 조건에 안 맞는 놈)을 만나는 순간"` 입니다.
		- 이것을 전문 용어로 `Index Range Scan`의 `Scan Stop Key`라고 부릅니다.
		- 인덱스의 리프 노드는 데이터가 가나다(또는 숫자) 순으로 정렬되어 있습니다. 
			- 이 정렬 상태가 바로 멈춤의 기준이 됩니다.
		- 조건 : `WHERE 사번 BETWEEN 10 AND 20`
			- 수직적 탐색으로 10번이 있는 리프 노드 시작점을 찾습니다.
			- 오른쪽으로 한 칸씩 이동하며 데이터를 읽습니다. (10, 11, 12...)
			- 그러다 21번을 만나는 순간! "어, 20보다 크네? 인덱스는 정렬되어 있으니 이 뒤로는 볼 필요도 없겠군!" 하고 즉시 스캔을 멈춥니다.
		- 이 "언제 멈추느냐"가 성능에 엄청난 영향을 줍니다.
 			- 인덱스가 있을 때 : 1억 건 중 조건에 맞는 10건만 딱 읽고 멈춥니다.
			- 인덱스가 없을 때 (Full Scan) : 10건을 다 찾았더라도, 뒤에 11번째 데이터가 또 있을지 모르니 1억 건을 마지막까지 다 뒤져야 끝납니다.
