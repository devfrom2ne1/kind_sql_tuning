...
- 그렇다면 시작지점을 찾기 어려워진다.. 왜?
	...
	- 그래서 **NULL은 '내용 없음'이 아니라, '가장 큰 값(무한대)'** 으로 취급해서 단어장의 맨 뒷장에 몰아넣는다고 생각하면 쉽습니다.
	- 우리가 WHERE 주문수량 < 100이라고 하면, 오라클은 단어장의 첫 페이지부터 100이 나오기 전까지만 슥 읽고 끝냅니다. 

	- 그런데 NVL(주문수량, 0) < 100이라고 하면 상황이 달라집니다.
		- 원래 데이터 : 단어장 맨 뒷장에 숨어있는 'NULL' 데이터들.
		- 컴퓨터의 생각 : "어? NULL을 0으로 바꾸라고? 그럼 맨 뒷장에 있던 애들이 갑자기 맨 앞(0의 위치)으로 와야 하네?"
		- 문제 발생 : 하지만 실제 인덱스 종이에는 여전히 NULL이 맨 뒤에 적혀 있습니다.
		- 결론 : 오라클은 "인덱스에 적힌 순서랑 내가 계산한 순서가 안 맞네? 그럼 인덱스(단어장) 안 쓰고 그냥 처음부터 끝까지 다 읽을래" 하고 포기해 버립니다.
	- 이걸 전문 용어로 **"인덱스를 타지 못한다(Full Table Scan)"** 라고 합니다.

	- 오라클의 기본 원칙은 **"NULL은 무한대다"** 입니다.
		- 오름차순 (1, 2, 3...) 인덱스 : 1, 2, 3... 순서로 기록하다가 맨 마지막 리프 블록에 NULL을 모아둡니다.
			- 그래서 "100보다 큰 값을 찾아라" 할 때 NULL까지 같이 딸려 나오기 쉽습니다.
		- 내림차순 (10, 9, 8...) 인덱스 : 반대로 NULL이 가장 크니까 맨 첫 번째 리프 블록에 NULL이 먼저 나옵니다.
		- 중요한 예외 (단일 컬럼): 만약 인덱스를 주문수량 하나에만 걸어두면, 오라클은 값이 NULL인 행은 아예 인덱스에 저장조차 안 합니다.
		- 즉, 단어장에 아예 존재하지도 않으니 WHERE 주문수량 IS NULL이라고 검색해도 인덱스를 못 쓰고 전체를 다 뒤져야 하는 거죠.

- 즉, 오라클에서 NULL은 기본적으로 **맨 뒤(가장 큰 값)** 에 있다.
- 그래서 NVL을 쓰는 순간, 맨 뒤에 있어야 할 NULL이 맨 앞(0)으로 가야 하는 모순이 생겨서 인덱스 순서가 꼬입니다.
- 성능을 높이려면 NVL 대신 `WHERE (주문수량 < 100 OR 주문수량 IS NULL)` 처럼 인덱스를 그대로 쓸 수 있는 조건을 사용하는 게 좋습니다.
