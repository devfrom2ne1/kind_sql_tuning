...
- 인덱스는 정렬되어 있다. 
- 그래서 정렬연산이 있어도(= `ORDER BY` 가 있어도) 인덱스를 잘 타면, 실행계획에선 	`SORT ORDER BY` 없이 실행된다. 
- `INDEX (RANGE SCAN DESCENDING) OF ...` 를 보면 인덱스 자체로 내림차순, 오름차순을 시행함을 알 수 있다. 

### 2.2.5 ORDER BY 절에서 컬럼 가공

- `조건절`에 사용된 컬럼이 아닌, `ORDER BY` 나 `SELECT-LIST`의 컬럼을 가공해도 인덱스를 정상적으로 사용할 수 없는 경우가 있다.
- 인덱스에는 가공하지 않은 상태로 값이 저장되어 있기 때문에, ORDER BY 뒤에 컬럼을 가공하면 정렬 연산을 생략할 수 없게 된다.

#### (예시) `주문_PK 인덱스` : [주문일자 + 주문번호] 인 경우를 생각해보자
...
- `SORT ORDER BY` 연산이 생략되지 못하는 이유는?
	- `ORDER BY` 절 뒤에 원래 컬럼이 아닌 TO_CHAR 가공한 '주문번호'가 쓰여 있기 때문이다. 
	- `ORDER BY`에 쓰인 컬럼명은 테이블 컬럼명이 아닌 `SELECT-LIST`에 적힌 컬럼명이다. 
	- `SELECT-LIST`에서 가공된 컬럼명이 Alias로 적혀 있으면?
...
- 이때, 실행계획에는 `FIRST ROW`와 `INDEX RANGE SCAN (MIN/MAX)`가 나타난다.

#### (예시) [장비번호 + 변경일자 + 변경순번] 인덱스 : 변경순번은 문자열!
...
- 이 경우에는 정렬연산을 생략할 수 없다!
- 그 이유는 크게 두 가지인데, **'데이터 타입 불일치'** 와 '함수 사용으로 인한 정렬 순서 파괴' 때문입니다.

- **① 데이터 타입과 정렬 기준의 차이**
	- 인덱스는 VARCHAR2 타입에 맞춰 **문자열 방식(사전식)**으로 정렬되어 있습니다.
		- 인덱스 내 정렬	: '1', '10', '2', '20', '3' ...
		- to_number 변환 후 기대하는 정렬 : 1, 2, 3, 10, 20 ...
	- 오라클 입장에서는 인덱스가 '10' 다음에 '2'가 오는 순서로 정렬되어 있는데, 
	- 사용자가 이걸 숫자로 바꿔서 그중 가장 큰 값(MAX)을 달라고 하면, 
	- 인덱스 순서를 그대로 믿고 마지막 값을 가져올 수 없습니다. 
	- 결국 전체를 읽어서 숫자로 바꾼 뒤 다시 비교해야 합니다.

- **② 인덱스 컬럼 가공 (Index Column Transformation)**
	- 인덱스 컬럼인 변경순번을 to_number() 함수로 감싸버리면, 기존에 구성된 인덱스 구조를 직접 사용할 수 없습니다. 
	- 인덱스는 '가공되지 않은 원본 값'을 기준으로 만들어져 있기 때문입니다.

- 정렬 연산을 생략하려면?
	- 문자열 상태에서 MAX 구하기 (데이터 포맷이 일정한 경우)
	- 순번이 0001, 0002 처럼 자릿수가 고정되어 있다면, 숫자로 바꾸지 말고 문자열 상태로 MAX를 구한 뒤 마지막에 숫자로 변환하세요.
	- 이 방식은 인덱스 정렬 순서를 그대로 활용할 수 있습니다.

```sql
SELECT NVL(TO_NUMBER(MAX(변경순번)), 0)
FROM 테이블명
WHERE 장비번호 = :B1 AND 변경일자 = :B2;
```

> 만약 순번이 1, 2, 10 처럼 자릿수가 일정하지 않은 문자열이라면, MAX(문자열) 결과가 9보다 10이 작다고 판단(사전식)할 수 있으므로 주의해야 합니다.
...
- 생년월일 컬럼이 문자열인데, 조건절 비교값을 숫자형으로 표현한다면?
	- 오라클 DBMS는 자동으로 형변환 처리를 해준다.
	- `TO_NUMBER(생년월일) = 19881225`가 되어버려 테이블을 전체 스캔하게 된다.

#### 숫자형 vs 문자형 = 숫자형
- 숫자형 기준으로 문자형 컬럼을 변환한다. 
- `LIKE` 연산자는 반대이다! 
- `LIKE`가 문자열 비교 연산자이기 때문이다. 

#### 날짜형 vs 문자형 = 날짜형
- 날짜형 기준으로 문자형 컬럼을 변환한다. 

#### 사용자가 입력할 수도 있고 안할 수도 있는 옵션 조건 
