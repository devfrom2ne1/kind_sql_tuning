...
#### 헷갈리는 개념 및 용어
1. 클러스터 (Cluster)
	- 아파트 단지 부지입니다. (여러 집이 모여 살 땅)

2. 클러스터 테이블 (Cluster Table)
	- 그 땅에 들어설 **실제 집(데이터)** 입니다.
	- 여러 개의 테이블을 물리적으로 한곳에 모아 저장하는 방식입니다. (미리 Join해둔 효과를 볼 수 있음)
	- **"공통 컬럼(Cluster Key)을 가진 데이터를 서로 다른 테이블이라도 같은 블록에 모아두는 것"** 입니다.
	- '아빠', '엄마', '나'라는 데이터(테이블)가 각기 다르지만, '우리 집'이라는 클러스터 키를 기준으로 한 페이지(블록)에 같이 모여 있는 것과 같습니다.
		
3. 클러스터 인덱스 (Cluster Index)
	- 단지 입구에 있는 호수별 위치 안내도입니다.
	- 테이블당 단 하나만 존재할 수 있는 '정렬된 인덱스'입니다.
	- **"인덱스의 리프 페이지가 곧 데이터 페이지"** 입니다. 즉, 인덱스 자체가 테이블 데이터입니다.
	- 자체로 "국어사전" 그 자체입니다. 'ㄱ'을 찾으면 그 자리에 바로 단어 뜻이 적혀 있는 것과 같습니다.
	- 클러스터 인덱스도 B*Tree 인덱스 구조를 사용하긴 하지만, 테이블 레코드와 1:1 대응 관계를 갖진 않는다. 
	- 클러스터 인덱스는 키 값을 저장하는 첫 번째 데이터 블록을 가리킨다. 
	- 그래서 테이블 레코드와 1:M 구조를 갖는다. 

#### 클러스터 테이블 생성 순서
 
> 순서 : '클러스터 생성 → 클러스터 인덱스 생성 → 클러스터 테이블 생성 및 데이터 입력' 
>
