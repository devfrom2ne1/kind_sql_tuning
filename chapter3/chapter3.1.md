...
| 클러스터 인덱스 (Clustered Index) | 인덱스 클러스터 테이블 (Index Cluster) |
|---|---|
| 단일 테이블의 정렬 방식 | 여러 테이블 간의 물리적 결합 |
| 인덱스 끝에 데이터가 붙어 있음 | 클러스터 키가 같은 행들을 같은 블록에 저장 |
| 단일 테이블 내 범위 검색 최적화 | Join 성능 극대화 (미리 Join해둔 효과) |
| 별도의 클러스터 오브젝트가 없음 | 반드시 CREATE CLUSTER로 클러스터를 먼저 생성 |
