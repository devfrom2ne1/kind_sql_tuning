...
```sql
-- 1. 고객ID가 입력되었을 때 (인덱스 Range Scan)
select * from 거래
...
> [!NOTE]
> - Null인 데이터는 인덱스에 안 넣습니다.
> - 옵티마이저는 "인덱스만 봐서는 Null 데이터를 놓칠 텐데? 에이, 그냥 테이블 다 읽자." (성능 저하)
> - Union All을 사용하면 "인덱스로 찾을 수 있는 데이터만 인덱스로 찾게 해주기 때문에..." (성능 확보)
> - 하지만 null이 입력되면 전체조회되도록 짤 수는 없다? 맞지?
