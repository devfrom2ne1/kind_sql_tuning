...
```
 * 테이블: 도서관 전체 책장
 * 인덱스: [장르 + 저자] 순으로 정렬된 목록집
 * 찾는 조건: 장르 = '소설' AND 저자 = '김철수' AND 출판일 > '2023년' AND 내용에 '강아지'가 포함됨
```
...
- "목록집에서 어디부터 어디까지 읽을지 딱 정해주는 조건"
	- 설명 : 인덱스의 첫 번째, 두 번째 컬럼처럼 정렬 순서에 직접 관여하여 스캔의 시작과 끝을 결정합니다.
	- 예시 : 목록집이 '장르'순으로 되어 있으므로, 장르 = '소설' 조건을 보고 소설 섹션이 시작되는 페이지로 바로 이동합니다. 그다음 저자 = '김철수'를 보고 김철수 저자가 끝나는 지점까지만 읽기로 합니다.
	- 효과 : 이 조건이 좋을수록 읽어야 할 리프 블록(목록 페이지) 수가 획기적으로 줄어듭니다.

#### 2. 인덱스 필터 조건

- 테이블로 액세스 할지 말지 결정하는 조건

- "목록집엔 적혀 있는데, 범위를 줄여주지는 못하고 하나씩 대조해봐야 하는 조건"
	- 설명 : 인덱스 구성 컬럼이긴 하지만, 정렬 순서상 범위를 좁히지는 못하고 목록을 읽으면서 "맞나? 아니네" 하며 걸러내는 역할입니다.
	- 예시 : 인덱스 목록에 출판일도 적혀 있다고 가정합시다. 하지만 목록이 [장르+저자] 순이라서 2023년 이후 책들이 여기저기 섞여 있습니다. '소설-김철수' 구간을 읽으면서 출판일을 하나씩 확인하며 걸러냅니다.
	- 효과 : 테이블(본문)로 가기 전에 목록에서 미리 걸러내기 때문에, 테이블 액세스 횟수를 줄여줍니다.
...
- "목록집엔 정보가 없어서, 실제 책을 꺼내서 펼쳐봐야만 알 수 있는 조건"
	- 설명 : 인덱스에 없는 컬럼이 조건절에 있을 때입니다. 무조건 실제 테이블 블록에 접근(Table Access)한 뒤에야 확인할 수 있습니다.
	- 예시 : 내용에 '강아지'가 포함됨 조건입니다. 목록집(인덱스)에는 줄거리 내용이 없습니다. 결국 '소설-김철수-2023년이후'인 책들을 서가에서 직접 꺼내서(Table Access) 내용을 읽어보고 강아지가 없으면 버립니다.
