...
| 지연 시간 | $10^{-9}$초 (나노초) 단위 | $10^{-3}$초 (밀리초) 단위 |
데이터베이스(DBMS)는 필요한 데이터를 **메모리(Buffer Cache)**에 보관하려고 하지만, 메모리가 부족하거나 처음 접근하는 데이터는 반드시 디스크에서 읽어와야 합니다. 이 디스크 접근(I/O Call)이 수반하는 엄청난 속도 차이 때문에 성능 병목이 발생합니다.

2. 프로세스와 CPU '대기' (Waiting State)
SQL 쿼리를 실행하는 데이터베이스 **프로세스(또는 스레드)**가 디스크에 데이터를 요청하는 순간 다음과 같은 일이 일어납니다.
 * I/O 요청: 프로세스가 운영체제(OS)를 통해 디스크에 데이터 읽기를 요청합니다.
 * CPU 반환: 프로세스는 디스크 I/O가 완료될 때까지 CPU를 점유하고 기다리는 대신, CPU 사용 권한을 운영체제에 즉시 반환합니다.
 * 대기 상태 진입: 해당 프로세스는 '실행 중(Running)' 상태에서 '대기 중(Waiting)' 상태로 바뀝니다.
 * CPU 활용: CPU는 놀지 않고, 반환된 시간을 이용하여 디스크 I/O를 요청하지 않은 다른 프로세스의 작업을 처리합니다.
이 메커니즘은 CPU를 효율적으로 사용하기 위함이지만, I/O 요청이 많고 디스크 응답 시간이 길수록 프로세스가 대기 상태로 머무는 총 시간이 증가합니다.
> 💡 결론: SQL 쿼리를 완료하는 데 걸리는 시간(응답 시간)은 CPU를 사용하는 시간보다 **디스크 I/O 완료를 기다리는 시간(Wait Time)**이 압도적으로 길기 때문에 느려지는 것입니다.

3. 📉 성능 저하 해결의 핵심
- SQL 튜닝은 결국 I/O 대기 시간을 최소화하는 것에 초점을 맞춥니다.
 * 인덱스 활용: 쿼리가 테이블 전체를 읽지 않고 필요한 일부 데이터만 디스크에서 읽도록 유도하여 I/O 횟수(논리적 I/O)를 줄입니다.
 * 캐싱 최적화: 메모리 버퍼 캐시 크기를 늘려 디스크 접근 없이 메모리에서 데이터를 바로 가져오는 비율을 높입니다(물리적 I/O 감소).
 * 하드웨어 개선: 더 빠른 디스크(NVMe SSD 등)를 도입하거나 I/O 대역폭을 확장하여 I/O 요청 자체의 처리 속도를 높입니다.
