...
2. 프로세스와 CPU '대기' (Waiting State)
	- SQL 쿼리를 실행하는 데이터베이스 **프로세스(또는 스레드)** 가 디스크에 데이터를 요청하는 순간 다음과 같은 일이 일어납니다.
		1) I/O 요청 : 프로세스가 운영체제(OS)를 통해 디스크에 데이터 읽기를 요청합니다.
		2) CPU 반환 : 프로세스는 디스크 I/O가 완료될 때까지 CPU를 점유하고 기다리는 대신, CPU 사용 권한을 운영체제에 즉시 반환합니다.
		3) 대기 상태 진입 : 해당 프로세스는 '실행 중(Running)' 상태에서 '대기 중(Waiting)' 상태로 바뀝니다.
		4) CPU 활용 : CPU는 놀지 않고, 반환된 시간을 이용하여 디스크 I/O를 요청하지 않은 다른 프로세스의 작업을 처리합니다.
