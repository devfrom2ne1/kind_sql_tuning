...
- 도서관을 데이터베이스에 비유한다면?
	1. 테이블스페이스 = 도서관의 '구역' 하나
	2. 세그먼트 = 구역 내에 '책장' 하나
	3. 익스텐트 = 책장에 추가된 '선반' 한 칸
	4. 블록 = 책 한권에 들어있는 '종이' 한 장

#### 데이터 저장 과정 및 구조 (정리)
1. 저장 위치 확인 (Tablespace)
 * 데이터가 들어갈 테이블스페이스를 먼저 확인합니다.
 * 특징: 세그먼트를 담는 컨테이너이며, 물리적인 OS 파일인 여러 개의 데이터 파일로 구성됩니다.
2. 세그먼트 및 공간 할당 (Segment & Extent)
 * 해당 테이블의 실제 저장 덩어리인 세그먼트를 찾아갑니다.
   * 세그먼트: 데이터가 담기는 논리적 객체 (테이블, 인덱스 등).
   * 공간이 부족하면 익스텐트 단위를 추가로 할당받습니다.
 * 익스텐트: 공간 확장의 단위이며, **'연속된 블록들의 집합'**입니다.
   * 블록을 하나씩 추가하는 것보다 효율적이며, 연속적 배치 덕분에 읽기 성능이 향상됩니다.
   * 중요: 하나의 익스텐트는 하나의 테이블이 독점합니다.
3. 최소 단위 기록 (Data Block)
 * 비어있는 데이터 블록을 찾아 데이터를 기록합니다.
   * 데이터 블록: 실제 데이터 저장의 최소 단위 (논리적인 방).
   * 구조: 공통 헤더 + 여유 공간 + 데이터 공간.
   * 하나의 블록은 하나의 테이블이 독점합니다.
4. 물리적 기록 (Physical Write)
 * 메모리(RAM)의 데이터를 하드디스크에 영구 저장하는 단계입니다.
   * 더티 블록(Dirty Block): 메모리에만 수정사항이 반영되고 아직 디스크엔 기록되지 않은 블록.
   * Redo Log: 전원 차단 등에 대비해 변경 사항을 빠르게 남기는 '백업 메모'.
   * DBWR (Writer): 메모리의 더티 블록들을 실제 데이터 파일에 덮어쓰며 마무리합니다.
